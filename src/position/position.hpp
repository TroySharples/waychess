...
inline std::uint64_t get_attackers_white(const bitboard& bb, std::uint8_t mb) noexcept
{
    const std::uint64_t pieces_bb  { bb.boards[piece_idx::w_any] | bb.boards[piece_idx::b_any] };

    return (get_black_pawn_all_attacked_squares_from_mailbox(mb)    & bb.boards[piece_idx::w_pawn])
         | (get_king_attacked_squares_from_mailbox(mb)              & bb.boards[piece_idx::w_king])
...
inline std::uint64_t get_attackers_black(const bitboard& bb, std::uint8_t mb) noexcept
{
    const std::uint64_t pieces_bb  { bb.boards[piece_idx::w_any] | bb.boards[piece_idx::b_any] };

    return (get_white_pawn_all_attacked_squares_from_mailbox(mb)    & bb.boards[piece_idx::b_pawn])
         | (get_king_attacked_squares_from_mailbox(mb)              & bb.boards[piece_idx::b_king])
