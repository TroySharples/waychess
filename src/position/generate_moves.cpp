...
std::size_t generate_pseudo_legal_moves(const bitboard& bb, std::span<std::uint32_t> move_buf) noexcept
{
    ...
    // some sort of sorting algorithm on the moves vector before returning. We used to break this out into a function
    // for each piece, but we've unrolled it to hopefully encourage compiler optimisation.
    ...
    const auto to_move_boards { is_black_to_play ? bb.get_black_boards() : bb.get_white_boards() };

    // Pawn moves.
    {
        for (std::uint64_t to_move_pawns { to_move_boards[piece_idx::w_pawn] }; to_move_pawns; )
        {
            ...
            for (std::uint64_t attacks { (is_black_to_play ? get_black_pawn_all_attacked_squares_from_mailbox(pawn_mailbox) : get_white_pawn_all_attacked_squares_from_mailbox(pawn_mailbox)) & (opponent_pieces | bb.en_passent_bb) }; attacks; )
            {
                ...
                {
                    const std::uint32_t move { move::serialise_make(pawn_mailbox, std::countr_zero(attack), piece_idx::w_pawn, move::move_type::CAPTURE) };
            ...
            {
                const std::uint32_t move { move::serialise_make(pawn_mailbox, std::countr_zero(push), piece_idx::w_pawn, move::move_type::PAWN_PUSH, move::move_info::PAWN_PUSH_SINGLE)};
            ...
            {
                const std::uint32_t move { move::serialise_make(pawn_mailbox, std::countr_zero(push), piece_idx::w_pawn, move::move_type::PAWN_PUSH, move::move_info::PAWN_PUSH_DOUBLE) };
                move_buf[ret++] = move;
            }
    ...
    // King moves.
    {
        ...
        const std::uint64_t king_bitboard { to_move_boards[w_king] };
        ...
        for (std::uint64_t attacks { get_king_attacked_squares_from_mailbox(king_mailbox) & ~to_move_pieces }; attacks; )
        {
            ...
            {
                const std::uint32_t move { move::serialise_make(king_mailbox, std::countr_zero(attack), piece_idx::w_king, attack & opponent_pieces ? move::move_type::CAPTURE : 0) };
                move_buf[ret++] = move;
            }
    ...
    // Knight moves.
    {
        for (std::uint64_t to_move_knights { to_move_boards[w_knight] }; to_move_knights; )
        {
            ...
            for (std::uint64_t attacks { get_knight_attacked_squares_from_mailbox(knight_mailbox) & ~to_move_pieces }; attacks; )
            {
                ...
                {
                    const std::uint32_t move { move::serialise_make(knight_mailbox, std::countr_zero(attack), piece_idx::w_knight, attack & opponent_pieces ? move::move_type::CAPTURE : 0) };
                    move_buf[ret++] = move;
                }
    ...
    // Rook moves.
    {
        for (std::uint64_t to_move_rooks { to_move_boards[w_rook] }; to_move_rooks; )
        {
            ...
            for (std::uint64_t attacks { get_rook_attacked_squares_from_mailbox(rook_mailbox, all_pieces) & ~to_move_pieces }; attacks; )
            {
                ...
                {
                    const std::uint32_t move { move::serialise_make(rook_mailbox, std::countr_zero(attack), piece_idx::w_rook, attack & opponent_pieces ? move::move_type::CAPTURE : 0) };
                    move_buf[ret++] = move;
                }
    ...
    // Bishop moves.
    {
        for (std::uint64_t to_move_bishops { to_move_boards[w_bishop] }; to_move_bishops; )
        {
            ...
            for (std::uint64_t attacks { get_bishop_attacked_squares_from_mailbox(bishop_mailbox, all_pieces) & ~to_move_pieces }; attacks; )
            {
                ...
                {
                    const std::uint32_t move { move::serialise_make(bishop_mailbox, std::countr_zero(attack), piece_idx::w_bishop, attack & opponent_pieces ? move::move_type::CAPTURE : 0) };
                    move_buf[ret++] = move;
                }
    ...
    // Queen moves.
    {
        for (std::uint64_t to_move_queens { to_move_boards[w_queen] }; to_move_queens; )
        {
            ...
            for (std::uint64_t attacks { get_queen_attacked_squares_from_mailbox(queen_mailbox, all_pieces) & ~to_move_pieces }; attacks; )
            {
                ...
                {
                    const std::uint32_t move { move::serialise_make(queen_mailbox, std::countr_zero(attack), piece_idx::w_queen, attack & opponent_pieces ? move::move_type::CAPTURE : 0) };
                    move_buf[ret++] = move;
                }
